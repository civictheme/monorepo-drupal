...
Feature: Promo card render
  ...
  Scenario:XSS - Promo Card
    ...
    And "field_c_n_components" in "civictheme_page" "node" with "title" of "[TEST] Page Promo cards test" has "civictheme_manual_list" paragraph:
      ...
      | field_c_p_list_fill_width   | 0                                                |
    And "field_c_p_list_items" in "civictheme_manual_list" "paragraph" with "field_c_p_title" of "[TEST] Promo manual list" has "civictheme_promo_card" paragraph:
      | field_c_p_title    | <script id="test-promo-card--field_c_p_title">alert('[TEST] Promo card field_c_p_title')</script>                    |
      | field_c_p_subtitle | <script id="test-promo-card--field_c_p_subtitle">alert('[TEST] Promo card field_c_p_subtitle')</script>                 |
      | field_c_p_summary  | <script id="test-promo-card--field_c_p_summary">alert('[TEST] Promo card field_c_p_summary')</script>                        |
      | field_c_p_image    | [TEST] CivicTheme Image               |
      | field_c_p_topics   | [TEST] Topic 1                        |
      | field_c_p_link     | 0: Test link - 1: https://example.com |
      | field_c_p_theme    | light                                 |
    ...
    And I should not see an "script#test-promo-card--field_c_p_title" element
    And I should see the text "alert('[TEST] Promo card field_c_p_title')"
    And I should not see an "script#test-promo-card--field_c_p_subtitle" element
    And I should see the text "alert('[TEST] Promo card field_c_p_subtitle')"
    And I should not see an "script#test-promo-card--field_c_p_summary" element
    And I should see the text "alert('[TEST] Promo card field_c_p_summary')"
