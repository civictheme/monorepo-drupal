...
Feature: Navigation card render
  ...
  Scenario:XSS - Navigation Card
    ...
    And "field_c_p_list_items" in "civictheme_manual_list" "paragraph" with "field_c_p_title" of "[TEST] Navigation manual list" has "civictheme_navigation_card" paragraph:
      | field_c_p_image   | [TEST] CivicTheme Image               |
      | field_c_p_link    | 0: Test link - 1: https://example.com |
      | field_c_p_summary | <script id="test-navigation-card--field_c_p_summary">alert('[TEST] Navigation card field_c_p_summary')</script>                          |
      | field_c_p_theme   | light                                 |
      | field_c_p_title   | <script id="test-navigation-card--field_c_p_title">alert('[TEST] Navigation card field_c_p_title')</script>                 |

    When I visit "civictheme_page" "[TEST] Page Navigation cards test"
    And I should see an ".ct-navigation-card" element
    And I should not see an "script#test-navigation-card--field_c_p_title" element
    And I should see the text "alert('[TEST] Navigation card field_c_p_title')"
    And I should not see an "script#test-navigation-card--field_c_p_summary" element
    And I should see the text "alert('[TEST] Navigation card field_c_p_summary')"
