...
Feature: Subject card render
  ...
  Scenario:XSS - Subject Card
    ...
    And "field_c_p_list_items" in "civictheme_manual_list" "paragraph" with "field_c_p_title" of "[TEST] Subject manual list" has "civictheme_subject_card" paragraph:
      ...
      | field_c_p_link  | 0: Test link - 1: https://example.com |
      | field_c_p_theme | light                                 |
      | field_c_p_title | <script id="test-subject-card--field_c_p_title">alert('[TEST] Subject card field_c_p_title')</script>                  |

    When I visit "civictheme_page" "[TEST] Page Subject cards test"
    And I should see an ".ct-subject-card" element
    And I should not see an "script#test-subject-card--field_c_p_title" element
    And I should see the text "alert('[TEST] Subject card field_c_p_title')"
