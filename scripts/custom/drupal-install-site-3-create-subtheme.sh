...
pushd docroot/themes/custom/civic >/dev/null || exit 1
php civic-create-subtheme.php civic_demo "Civic Demo" "Demo sub-theme for a Civic theme."
[ ! -d docroot/themes/custom/civic_demo ] && echo "ERROR: Failed to create civic_demo sub-theme." && exit 1
popd >/dev/null || exit 1

if command -v npm &> /dev/null; then
  pushd docroot/themes/custom/civic_demo >/dev/null || exit 1
...
echo "  > Install a civic_demo theme."
$drush ${DRUSH_ALIAS} theme:enable civic_demo -y

echo "  > Making civic_demo a default theme."
$drush ${DRUSH_ALIAS} config-set system.theme default civic_demo
